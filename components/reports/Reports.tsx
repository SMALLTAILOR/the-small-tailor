...
const ReportForm: React.FC<ReportFormProps> = ({ onSubmit, selectedUserId, setSelectedUserId, employees, startDate, setStartDate, endDate, setEndDate }) => (
    <form onSubmit={onSubmit} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6 pb-6 border-b items-end">
        ...
        <label htmlFor="employee" className="block text-sm font-medium text-slate-700">Employee</label>
        <select id="employee" value={selectedUserId} onChange={e => setSelectedUserId(e.target.value)} className="mt-1 block w-full p-2 border border-slate-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm" required>
         ...
         <div className="border-b border-slate-200">
            <nav className="-mb-px flex space-x-8" aria-label="Tabs">
                <button onClick={() => setActiveTab('payroll')}
                    className={`${activeTab === 'payroll' ? 'border-primary-500 text-primary-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>
                    Payroll Report
                </button>
                 <button onClick={() => setActiveTab('attendance')}
                    className={`${activeTab === 'attendance' ? 'border-primary-500 text-primary-600' : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'} whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm`}>
                    Attendance Report
                </button>
            </nav>
        </div>
        ...
        <div className="mt-6">
            ...
            <div className="animate-fade-in">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={handleDownloadPayroll} className="flex items-center gap-2 bg-slate-200 text-slate-700 px-3 py-1 rounded-md hover:bg-slate-300 text-sm">
                        <span className="w-4 h-4">{ICONS.download}</span>
                        Download CSV
                    </button>
                </div>
                <p className="text-sm text-slate-600 mb-4">From {payrollReport.startDate} to {payrollReport.endDate}</p>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="bg-slate-50 p-4 rounded-lg text-center">
                        <p className="text-sm text-slate-500">Total Days Present</p>
                        <p className="text-2xl font-bold">{payrollReport.totalPresentDays}</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg text-center">
                        <p className="text-sm text-slate-500">Total Work Entries</p>
                        <p className="text-2xl font-bold">{payrollReport.workEntries.length}</p>
                    </div>
                    <div className="bg-slate-50 p-4 rounded-lg text-center">
                        <p className="text-sm text-slate-500">Total Piece Rate Earnings</p>
                        <p className="text-2xl font-bold text-emerald-600">₹{payrollReport.totalPieceRateEarnings.toFixed(2)}</p>
                    </div>
                </div>
                            ...
                            {payrollReport.workEntries.map(entry => {
                                ...
                                return (
                                    <tr key={entry.id}>
                                        ...
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{entry.trackingNumber}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-900">{getOperationName(entry.operationId)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{entry.quantity}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">₹{rate.toFixed(2)}</td>
                                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">₹{amount.toFixed(2)}</td>
                                    </tr>
                                )
                            })}
                            {payrollReport.workEntries.length === 0 && (
                                <tr><td colSpan={6} className="text-center py-4 text-slate-500">No work entries found for this period.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
            ...
            <div className="animate-fade-in">
                <div className="flex justify-between items-center mb-2">
                    ...
                    <button onClick={handleDownloadAttendance} className="flex items-center gap-2 bg-slate-200 text-slate-700 px-3 py-1 rounded-md hover:bg-slate-300 text-sm">
                        <span className="w-4 h-4">{ICONS.download}</span>
                        Download CSV
                    </button>
                </div>
                <p className="text-sm text-slate-600 mb-4">From {attendanceReport.startDate} to {attendanceReport.endDate}</p>

                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                     <div className="bg-slate-50 p-4 rounded-lg text-center md:col-start-2">
                        <p className="text-sm text-slate-500">Total Days Present (Approved)</p>
                        ...
                        <tbody className="bg-white divide-y divide-slate-200">
                            {attendanceReport.attendanceRecords.map(rec => (
                                <tr key={rec.id}>
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{rec.checkInTime}</td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm">
                                         <span className="px-2 py-1 text-xs font-medium rounded-full text-emerald-800 bg-emerald-100">
                                            {rec.status}
                                        </span>
                                    </td>
                                </tr>
                            ...
                            {attendanceReport.attendanceRecords.length === 0 && (
                                <tr><td colSpan={3} className="text-center py-4 text-slate-500">No approved attendance records found for this period.</td></tr>
                            )}
                        </tbody>
                    </table>
                </div>
